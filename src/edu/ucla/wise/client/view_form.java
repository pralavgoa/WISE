...
public class view_form extends HttpServlet
{
	...
	public void service(HttpServletRequest req, HttpServletResponse res)
		...
		if(p_output!=null && !p_output.equalsIgnoreCase(""))
		{
			out.println("<html><head>" +
					"<link rel='stylesheet' href="+"'styleRender?css=style.css' type='text/css'>\n"+
					"<script type='text/javascript' language='JavaScript1.1' src='"+ Surveyor_Application.shared_file_url +"survey.js'></script>" +
					"<script type='text/javascript' src='"+Surveyor_Application.shared_file_url+"jquery-1.7.1.min.js'></script>");
		...
		html_content.append("<title>Web-based Interactive Survey Environment (WISE)</title>");
		html_content.append("<meta http-equiv='Content-Type' content='text/html; charset=iso-8859-1'>"
				...
				+ "</script>");
		html_content.append("<link type='text/css' href='styleRender?css=style.css' rel='stylesheet'>");
		html_content.append("</head>");
		html_content.append("<body onload='javascript: setFields();check_preconditions();'>");
	...
	public String getProgressDivContent(User theUser, HttpSession session)
	{
		...
		if(theUser.currentSurvey.allow_goback)
			progress_bar.append(theUser.currentSurvey.print_progress(theUser.currentPage));
		else
			progress_bar.append(theUser.currentSurvey.print_progress(theUser.currentPage, completed_pages));
	...
	public String getPageHTML(User theUser)
	{
		...
		if(p_output!=null && !p_output.equalsIgnoreCase(""))
		{
			page_html.append("<script type='text/javascript' language='JavaScript1.1' src='"+ Surveyor_Application.shared_file_url +"survey.js'></script>");
			page_html.append("<script type='text/javascript' src='"+Surveyor_Application.shared_file_url+"jquery-1.7.1.min.js'></script>");
			page_html.append(p_output);
		}
		else
		{
			//redirect to the next page by outputting hidden field values and running JS submit()
			page_html.append("<form name='mainform' method='post' action='readform'>");
			page_html.append("<input type='hidden' name='action' value=''>");
			if ( (theUser.currentSurvey.is_last_page(theUser.currentPage.id)) || (theUser.currentPage.final_page) )
				page_html.append("<input type='hidden' name='nextPage' value='DONE'>");
			else
			{
				//if the id of the next page is not set in the survey xml file (with value=NONE),
				//then get its id from the page hash table in the survey class.
				if ( theUser.currentPage.next_page.equalsIgnoreCase("NONE"))
					page_html.append("<input type='hidden' name='nextPage' value='"+ theUser.currentSurvey.next_page(theUser.currentPage.id).id+"'>");
				else
					//otherwise, assign the page id directly to the form
					page_html.append("<input type='hidden' name='nextPage' value='"+ theUser.currentPage.next_page+"'>");
			}
			page_html.append("</form>");
			page_html.append("<script LANGUAGE='JavaScript1.1'>document.mainform.submit();</script>");  
		}
