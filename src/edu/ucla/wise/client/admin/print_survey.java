...
public class print_survey extends HttpServlet {
	...
	try{
	    ...
	    String initErr = Surveyor_Application.check_init(req
		    .getContextPath());
	    if (initErr != null) {
		out.println(initErr + "<p> Servlet called: Print Survey</p>"
			+ Surveyor_Application.initErrorHtmlFoot);
		WISE_Application.log_error("WISE Surveyor Init Error: "
		...
		res.sendRedirect("admin_print_survey?s=" + survey_id);
		...
		out.println("<link rel='stylesheet' href='"
			+ study_space.style_url + "print.css' type='text/css'>");
		out.println("</head>");
	    ...
	    log.error("Client print survey has IOException");
	}
    }
