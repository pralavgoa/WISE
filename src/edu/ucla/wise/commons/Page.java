...
public class Page {
	...
	public ArrayList<Repeating_Item_Set> repeating_items 
												= new ArrayList<Repeating_Item_Set>();
	...
	public Page(Node n, Survey s) {
		try {
			...
			System.out.println("The number of children are: "+page_item_count);
			...
			for (int i = 0, k = 0; i < nodelist.getLength(); i++) {
				...
				if (Page_Item.IsPageItemNode(node1)) {
					...
					if(Repeating_Item_Set.IsRepeatingItemSetNode(node1))
					{
						Repeating_Item_Set repeating_set = Repeating_Item_Set.MakeNewItem(node1);
						if(repeating_set == null)
						{
	...
	/*
	 * Pralav: adding method to return repeating sets, used for database table creation
	 */
...
//P			if (!write_page)
//P				return s;
			condition = cond.getJs_expression();
		}
		...
		s += "</script>";
		s += "<script src='./javascript/bootstrap-tooltip.js' type='text/javascript'>";
		s += "</script>";
		/*************************************************************/
...
//remove and use jquery		s += "<body onload='javascript: setFields(); check_preconditions();'>";
...
//P			s += "<p>";
			...
			s+=items[i].render_form(theUser,i) + "\n";
		}
