...
public class Page {
	...
	try {
		...
		if (Page_Item.IsPageItemNode(node1)) {
			...
			Repeating_Item_Set repeating_set = Repeating_Item_Set
				.MakeNewItem(node1);
		...
		if (items[i] instanceof Repeating_Item_Set) {
		    // ignore these
		} else {
    ...
    /*
     * Pralav: adding method to return repeating sets, used for database table
     * creation
     */
	...
	if (!title.equalsIgnoreCase("NONE"))
	    s += "<title>" + title + "</title>";
	s += "<LINK href='" + "styleRender?app=" + ss.study_name
