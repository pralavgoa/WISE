...
public class Repeating_Item_Set extends Page_Item{
	...
	public Repeating_Item_Set(Node i_node)
	{
		...
		System.out.println("Repeating point 1");
		try{
			id = i_node.getAttributes().getNamedItem("ID").getNodeValue();
			System.out.println("Repeating point 2"+id);
			//			title = i_node.getAttributes().getNamedItem("Title").getNodeValue();

			System.out.println("Repeating point 3");
			...
			for(int i=0; i<node_list.getLength();i++)
			{
				...
				{
					if(child_node.getNodeName().equalsIgnoreCase("Precondition"))
					{
		...
		{
			//something with the exception Pralav
			WISE_Application.log_error("WISE - survey parse failure at Repeating Item Set ["
					+ id + "] " + e.toString() + "\n" + this.toString(), null);
			return;
		}
	}
...
//		html_content.append(get_javascript_html());
		html_content.append("<div id=q"+item_index+" class='repeating_item_set'>");
...
//				"Add a "+get_name_for_repeating_set()+" </h2>");
		html_content.append("<div style='display: block; background-color:#353535;'>");
		html_content.append("<input type='text' class='repeat_item_name span3' placeholder='Enter "+get_name_for_repeating_set()+"' />");
		html_content.append("<a href='#' class='add_repeat_instance btn btn-primary btn-medium'>Add</a>");
		html_content.append("</div>");
		html_content.append("<div class='add_item_to_repeating_set'>");
		for(int i=0;i<item_set.size();i++)
		{
			html_content.append(item_set.get(i).render_form(i_user, 100*item_index+i));//100 is multiplied to get diff div id, not good
		}
		html_content.append("<a href='#' class='repeat_item_save'><b>Save this item</b></a>");
		html_content.append("</div>");
		html_content.append("<div class = 'repeating_question' Name="+get_name_for_repeating_set()+">");
		html_content.append("</div>");
		html_content.append("</div>");
		...
		public char getValueType(){
			return 'z'; //arbitrary string to satisfy the caller. Do something about this!!!
		}
		...
		public static Repeating_Item_Set MakeNewItem(Node n) {
			...
			try {
				...
				WISE_Application
				.log_error("PAGE ITEM Creation attempt failed for "
						+ nname + ": " + e, null);
			}
		...
		public static boolean IsRepeatingItemSetNode(Node n) {
			...
			try {
				...
				WISE_Application.log_error("Repeating Item Set test attempt failed for "
						+ n + ": " + e, null);
			}
			return answer;
		}
