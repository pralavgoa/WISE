...
/**
This class is a subclass of Page_Item and represents a Precondition object on the page
*/
public class Condition extends Page_Item
{
/** Instance Variables */
...
/** constructor: parse a Precondition node from XML */ 
  public Condition(Node n)
  {
      ...
      try
      {
          ...
          {
              //parse the leaf node
        	  // note XML Schema enforces order of: field, operator, (constant OR field)
        	  pre_field = node1.getFirstChild().getNodeValue();
      		  js_expression.append("(a['"+pre_field+"']");
             	if (pre_field.equals(""))
          		   throw new Exception("Invalid Precondition: Empty field name before "+node2_name);
             	//represent comparison operator as integer, since eval needs 'switch,' which can't take strings
             	...
             	else if (node2_name.equalsIgnoreCase("neq"))
             	{
               	...
               	js_expression.append(" != ");
          		   ...
          		   throw new Exception("Invalid operator in Precondition: "+node2_name);
             	...
             	else if(node3_name.equalsIgnoreCase("field"))
             	{
                 	...
                 	if (pre_field_second.equals(""))
              		   throw new Exception("Invalid Precondition: Empty field name after "+node1_name);
             	}
             	else
          		   throw new Exception("Invalid comparator in Precondition: "+node3_name);
         	}
         	...
         	else if (node1_name.equalsIgnoreCase("apply"))
         	{
             	...
             	else if (node2_name.equalsIgnoreCase("or"))
             	{
          		   ...
          		   throw new Exception("Invalid boolean operator in Precondition: "+node2_name);
             	...
             	if (node3_name.equalsIgnoreCase("apply"))
             	{
            	  ...
            	  throw new Exception("Invalid righthand predicate in Precondition: "+node3_name);
          }
          else
        	  throw new Exception("Invalid Precondition node starting at: "+node1_name);
            
      }//end of try
      catch (Exception e)
      {
        WISE_Application.log_error("WISE - CONDITION parse: "+e.toString(), null);
        return;
      }
  }
...
/** recursively execute the condition check */
  public boolean check_condition(User u)
  {
      ...
      //if the recursion has not reached to the leaf level, then continue to check the condition
      if(operatr_int < 10) //not a leaf node
      {
		  ...
		  switch (operatr_int)
		  {
		  	case 1: result = (apply_result && apply2_result); break;
		  	case 2: result = (apply_result || apply2_result); break;
		  }
      }
      ...
      else
      {
         //attempt lookup of value for field name(s) from user
          ...
          if (pre_field_second.equals("")) 
        	  ...
        	  result = compare(fieldVal1, operatr_int, u.get_field_value(pre_field_second));
      }
      return result;
  }
  ...
  private boolean compare(Integer fieldInt1, int op, Integer fieldInt2)
  {
      ...
      switch (op)
      {
          case 11: result = (fieldVal1 > fieldVal2); break;
          case 12: result = (fieldVal1 < fieldVal2); break;
          case 13: result = (fieldVal1 >= fieldVal2); break;
          case 14: result = (fieldVal1 <= fieldVal2); break;
          case 15: result = (fieldVal1 == fieldVal2); break;
          case 16: result = (fieldVal1 != fieldVal2); break;
          default: break;       
      }
      return result;
  }

/** Check precondition for each member of a SubjectSet (to display stems in a question block) */ 
  public boolean[] check_condition(String SubjectSetName, String[] SubjectSet, User theUser)
  {
      ...
      if(operatr_int < 10) //not a leaf node
      {
    	  ...
    	  boolean[] apply_result = cond.check_condition(SubjectSetName, SubjectSet, theUser);
    	  boolean[] apply2_result = cond2.check_condition(SubjectSetName, SubjectSet, theUser);
          //apply comparison to each paired element 
 		  switch (operatr_int)
		  {
		  	case 1: for(i=0; i<SubjectSet.length; i++)
		  		resultVector[i] = (apply_result[i] && apply2_result[i]); break;
		  	case 2: for(i=0; i<SubjectSet.length; i++)
		  		resultVector[i] = (apply_result[i] || apply2_result[i]);break;
		  }
      }
      else
      {
         ...
         int[] pre_fv = get_valuelist(theUser, SubjectSetName, SubjectSet, pre_field);
         //if the comparison pair is field vs. field, then get another value set of the 2nd field name
         if(pre_field_second.equals(""))
         {
             i=0;
             while(i<SubjectSet.length)
             {
//                resultVector[i] = compare(pre_fv[i], operatr_int, int_constant);
                i++;
             }
         }
         else
         {
            int[] pre_fv2 = get_valuelist(theUser, SubjectSetName, SubjectSet, pre_field_second);
            i=0;
            while(i<SubjectSet.length)
            {
//               resultVector[i] = compare(pre_fv[i], operatr_int, new Integer (pre_fv2[i])); //Quick patch to compile here
               i++;
            }
         }
      }
...
/** search by field name, get the value set from the subject data table that user conducted */
 public int[] get_valuelist(User theUser, String SubjectSetName, String[] SubjectSet, String field_name)
    {
...
//              {
//                  String current_key=SubjectSet[i].substring(SubjectSet[i].lastIndexOf("_")+1);
//                  //Study_Util.email_alert("CONDITION GET VALUELIST: - check keys: " + current_key);
//                  list_v[i]= (String) DataSet.get(current_key);
//                  if(list_v[i]==null || list_v[i].equalsIgnoreCase("null") || list_v[i].equalsIgnoreCase("") )
//                      list_value[i]=0;
//                  else
 ...
 public String toString()
   {
     if (operatr_int < 10) //not a leaf node
    	 ...
    	 return "leaf node: (" +pre_field+ ") op (" + int_constant.toString() +")<br>";
     else
    	 return "leaf node: (" +pre_field+ ") op (" + pre_field_second +")<br>";
   }
