...
public class User_DB_Connection {
	...
	public int storeMainData(String[] names, char[] valTypes, String[] vals) {
		...
		try {
			...
			WISE_Application.log_error("WISE - PAGE Store error ["
					+ theUser.id + "] query (" + sql + "): " + e.toString(),
					null);
		}
		try {
			stmt.close();
		} catch (Exception e) {
			WISE_Application.log_error("WISE - PAGE Store closing error: "
					+ e.toString(), null);
		}
	...
	public void begin_survey(String pageID) {
		try {
			...
			WISE_Application.log_error(
					"Databank SETUP STATUS:" + e.toString(), null);
		}
	}
	...
	public void insert_update_row_repeating_table(
			String table_name, String row_id,
			...
			Hashtable<String,String> name_type)
	{
		...
		while(e_iterator.hasMoreElements())
		{
			...
			comma_sepd_update_string.append(column_name+"=VALUES("+column_name+"),");
		...
		if(comma_sepd_column_names.charAt(comma_sepd_column_names.length()-1)==',')
			comma_sepd_column_names.setCharAt(
					comma_sepd_column_names.length()-1, ' ');
		if(comma_sepd_column_values.charAt(comma_sepd_column_values.length()-1)==',')
			comma_sepd_column_values.setCharAt(
					comma_sepd_column_values.length()-1, ' ');
		if(comma_sepd_update_string.charAt(comma_sepd_update_string.length()-1)==',')
		...
		if(row_id!=null)
			sql_statement.append(" (instance,invitee, ");
		else
			sql_statement.append("(invitee, ");
		...
		try {
			...
			WISE_Application.log_error("WISE - Repeat Item Store error ["
					+ theUser.id + "] query (" + sql_statement.toString() + "): " + e.toString(),
					null);
		}
		try {
			statement.close();
		} catch (Exception e) {
			WISE_Application.log_error("WISE - Repeat Item Store closing error: "
					+ e.toString(), null);
		}
	...
	public String get_all_data_for_repeating_set(String repeating_set_name)
	{
		...
		try{
			...
			while(rs.next()) {
				...
				for (column_index = 1; column_index <= columns; column_index++) {
					...
					if (col_name == null)
						throw new Exception("can't get column name " + column_index);
					ans = rs.getString(col_name);
					...
					if(ans != null)
					{
						javascript_array_response.append("\""+col_name+"\"");
						javascript_array_response.append(":");
						javascript_array_response.append("\""+ans+"\"");
					}
					else {//dont add;
					...
					else
					{
						...
						if(javascript_array_response.charAt(javascript_array_response.length()-1)==',')
						{
							javascript_array_response.deleteCharAt(javascript_array_response.length()-1);
						}
						javascript_array_response.append("}],");
					}
				}
		...
		catch(Exception e)
		{
			...
			WISE_Application.log_error("USER_DB REPEATING SET after " + column_index
					+ " cols read: " + e.toString(), e);
		}
	...
	public Hashtable get_all_data() {
		...
		try {
			...
			WISE_Application
			.log_error("USER GET DATA: " + e.toString(), null);
		}
		return h;
	}
	...
	public void set_consent(String answer) {
		try {
			...
			WISE_Application.log_error("USER_DB SET CONSENT:" + e.toString(),
					e);
		}
	}
	...
	public boolean check_consent() {
		...
		try {
			...
			WISE_Application.log_error("USER CHECK CONSENT:" + e.toString(),
					e);
		}
		return resultp;
	}
