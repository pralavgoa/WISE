...
public class Question_Block extends Page_Item {
	public static String Sql_datatype = "int(6)";

	/** Instance Variables */
	public String instructions = "NONE";
	public Response_Set response_set;
	...
	public Question_Block(Node n) {
		...
		try {
			...
			/*P			// parse subject stem
			// count the total number of the subject stems
			for (int i = 0; i < nodelist.getLength(); i++) {
				if (nodelist.item(i).getNodeName().equalsIgnoreCase("Sub_Stem"))
					num_stems++;
			} 
			// declare the string array for stem name & value based on the
			// subject stem size
			...
			for (int i = 0, j = 0; i < nodelist.getLength(); i++) {
				if (nodelist.item(i).getNodeName().equalsIgnoreCase("Sub_Stem")
					...
					transformer.transform(new DOMSource(node),
							new StreamResult(sw));
					String stem_type = nodelist.item(i).getNodeName().toUpperCase();
					//P					stems[j] = sw.toString();
					stems.add(this.new Stem_Differentiator(stem_type,sw.toString()));
	...
	public String[] listFieldNames() {
		...
		return (String[])stem_fieldNames.toArray(new String[stem_fieldNames.size()]);
	}
	...
	/**
	 * Renders the {@link Question_Block} HTML {@link Page_Item}. Renders a
	...
	public String make_html() {
		...
		classified_level_columns += "<td class=\"header-row\" colspan=" + levels + " width='60%'>";
		classified_level_columns += "<table cellpadding='3' border='0' width='100%' cellspacing='0'>";
		classified_level_columns += "<tr class='shaded-bg'>";// make one row 'stead of many
		...
		for (int i = 0; i < stems.size(); i++) {
			boolean is_Sub_Head = stems.get(i).stem_type.equalsIgnoreCase("Sub_Head");
			boolean is_Sub_Stem = stems.get(i).stem_type.equalsIgnoreCase("Sub_Stem");
			if(i==0)
			{
					...
					if(is_Sub_Stem)
					{
						...
						for (int j = startV, k = 0; j < len + startV; j++, k++) {
							if (((String) response_set.values.get(k))
								...
								s += "<input type='radio' name='"
										+ stem_fieldNames.get(i).toUpperCase()
										+ "' value='" + response_set.values.get(k)
										+ "'>";
								s += "</center></td>";
								num = num + 1;
							}
				...
				if(is_Sub_Head)
				{
					s += "<td class=\"sub_head\">" + stems.get(i).stem_value + "</td>";
					if (levels == 0) {
						s += no_classified_level_columns;
						s += "</tr>";
					} // if classified level is required for the question block
	...
	public String print_survey() {
		...
		for (int i = 0; i < stems.size(); i++) {
			...
			if (levels == 0) {
				for (int j = startV, k = 0; j < len + startV; j++, k++) {
					s += "<td align=center>";
					s += "<img src='" + WISE_Application.rootURL + "/WISE"
	...
	public String render_results(Page pg, Data_Bank db, String whereclause,
		...
		if (levels == 0) {
			...
			for (int j = 0; j < response_set.responses.size(); j++) {
				...
				t2 = String.valueOf(j + startValue);
				s += "<td bgcolor=#BA5D5D width='" + width
						+ "%'><b><center>" + t2
						+ "</center></b></td>";
			}
		}
		...
		for (int i = 0; i < stems.size(); i++) {
			...
			String subj_ans = (String) h1.get(stem_fieldNames.get(i).toUpperCase());
				...
				// if the question block has the subject set ref
					...
					if (user_id != null && !user_id.equalsIgnoreCase("")) {
						sql = "select "
								...
								.substring((stem_fieldNames.get(i)
										.lastIndexOf("_") + 1))
										+ " and invitee=" + user_id;
						boolean dbtype = stmt.execute(sql);
						ResultSet rs = stmt.getResultSet();
