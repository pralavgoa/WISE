 ...
 * @author ssakdeo
 */
public class WISE_Application {

	static Logger log = Logger.getLogger(WISE_Application.class);
	...
	// public static
	public static java.util.Hashtable<String, Surveyor_Application> AppInstanceTbl = new java.util.Hashtable<String, Surveyor_Application>();
	...
	public static String initialize() throws IOException {
		...
		localProps = ResourceBundle.getBundle(localPropPath, Locale
				.getDefault());
		try {
		...
		Study_Space.setupStudies(); // set up Study_Space class -- pre-reads
		// from sharedProps
	 ...
	 * the {@link WISE_Application} class. But proper messagebody and exception
	...
	// session if null
	{
		...
		/*Pralav has commented old code
		Properties sys_props = System.getProperties();
		// setup the mail server in system properties
		sys_props.put("mail.smtp.host", email_host);
		sys_props.put("mail.smtp.auth", CommonUtils.isEmpty(pwd) ? "false"
				: "true");
		Authenticator auth = CommonUtils.isEmpty(pwd) ? null
				: new VarAuthenticator(uname, pwd);
